    ...
    forAll(uTau, facei)
    {
        ...
        if (ut > ROOTVSMALL)
        {
            ...
            do
            {
                ...
                scalar fkUu = exp(kUu);
		scalar f = - ut*y[facei]/nuw[facei] + 1/E_*kUu*fkUu/ut;

		scalar df = y[facei]/nuw[facei] + 1/E_*kUu*fkUu/ut;
...
) const
{
    ...
    forAll(uTau2, facei)
    {
		...
		scalar f = - ut2/magUp[facei] + (kappa_*(yp-5)*yp - (30-yp)*lg)/(25*yp*lg);
		scalar df1 = ((- 2*kappa_*yp + 5*kappa_ - lg)*y[facei]/nuw[facei] + (30-yp)/ut2)/(25*yp*lg);
		scalar df2 = - (- kappa_*(yp-5)*yp + (30-yp)*lg)*(yp/ut2 + lg*y[facei]/nuw[facei])/(25*sqrt(yp*lg));

		scalar df = 1/magUp[facei] + df1 + df2;
...
// NEW STUFF BETWEEN THIS //
    const fvPatchVectorField& Uw = turbModel.U().boundaryField()[patchi]; // Velocity @wall
    const scalarField magUp(mag(Uw.patchInternalField() - Uw)); // ??
